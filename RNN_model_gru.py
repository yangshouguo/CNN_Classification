#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2018/10/27 下午12:01
# @Author  : 杨寿国
# @mail    : 891584158@qq.com
# @File    : RNN_model_gru.py
# @intro:
# !/usr/bin/env python
# -*- coding: utf-8 -*-
...
class TextCNN(object):
    def __init__(self
        ...
        self._rnn_out = self.GRU(n_hidden=self._rnn_hidden_units, out_size=self._rnn_output,
                                 x=self.embedding_chars_expanded)
    ...
    def GRU(self, n_hidden, out_size, x):

        x = tf.reshape(x, [self.batchsize, -1, self.embedding_size * self.seq_width])
    ...
    def BiLSTM(self, inputx, hidden_size, out_size, batch_size, time_step):
        ...
        with tf.variable_scope("rnn_{}".format(time_step)):
            ...
            weights = tf.get_variable("lstm_weights_{}".format(time_step), [2 * hidden_size, out_size],
                                      dtype=tf.float32,
                                      initializer=tf.random_normal_initializer(mean=0, stddev=1)
                                      )
