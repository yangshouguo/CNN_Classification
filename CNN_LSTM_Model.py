...
class TextCNN(object):
    def __init__(self
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                ...
                rnninfo = self.LSTM(self._hidden_size, self._rnn_output, pooled, time_step=seq_len - filter_size + 1)
    ...
    def LSTM(self, n_hidden, out_size, x, time_step):

        x = tf.reshape(x, [self.batchsize, time_step, -1])

        with tf.variable_scope('LSTM_{}'.format(time_step)):

            rnn_cell = tf.nn.rnn_cell.LSTMCell(n_hidden, name="LSTM")
